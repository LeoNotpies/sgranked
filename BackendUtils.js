...
class Database {
  ...
  async connectWithRetry(maxRetries = 5, retryDelay = 5000) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        Console.log("Database", `Trying to connect (attempt ${attempt}/${maxRetries})...`);
        ...
        lastError = error;
        Console.error("Database", `Error in attempt ${attempt}:`, error.message);
        
        if (attempt < maxRetries) {
          Console.log("Database", `Waiting ${retryDelay/1000} seconds to try again...`);
...
async autoPopulateSharedData() {
    try {
        ...
        Console.error('Population', 'Error when popular collections:', error);
    }
  }
  async createIndexes() {
...
database.connectWithRetry(5, 5000).catch(err => {
  ...
  Console.error("Database", "Verify that the MongoDB Atlas cluster is online and that the IP address is open.");
  Console.error("Database", "Visit: https://cloud.mongodb.com -> Network Access -> Add IP Address -> 0.0.0.0/0");
  process.exit(1);
});
...
class UserModel {
  ...
  static async create(deviceId, platformData = {}) {
    ...
    const username = `StumbleRanked#${userId}`;
...
async function generatePhotonJwt(user) {
  ...
  const options = { expiresIn: '30d', issuer: 'StumbleCrownsPhoton' };
...
class UserController {
  static async login(req, res) {
    try {
      ...
      if (!user) {
        Console.log("Login", "Creating a new user with the deviceId: " + DeviceId);
        user = await UserModel.create(DeviceId, { steamTicket: SteamTicket, scopelyId: ScopelyId, Version });
      }

      // If a newUsername was sent, update the user's username.
      if (newUsername && newUsername !== user.username) {
        ...
        if (newUsername.length < 4 || newUsername.length > 12) {
          return res.status(422).json({ message: 'Username must be between 4 and 12 characters' });
        }
        ...
        Console.log("Login", "Username updated to: " + newUsername + " for user: " + user.stumbleId);
      }
...
class RoundController {
  static async finishRound(req, res) {
    try {
      ...
      const rewards = {
        crowns: round === '3' ? 1 : 0,
        skillRating: round === '3' ? 20 : 10,
        experience: 100,
        gems: round === '3' ? 10 : 0
      };

    if (rewards.gems > 0) {
      await UserModel.addBalance(user.deviceId, "gems", rewards.gems);
    }
...
class EconomyController {
  static async purchase(req, res) {
    try {
      ...
      await UserModel.removeBalance(user.deviceId, price.Currency, price.Amount);
  ...
  static async purchaseGasha(req, res) {
    try {
      ...
      await UserModel.removeBalance(user.deviceId, 'gems', price.Amount);
  ...
  static async purchaseLuckySpin(req, res) {
    try {
      ...
      const freeSpins = UserModel.getBalanceAmount(user, 'default_free_spins');
      const adSpins = UserModel.getBalanceAmount(user, 'default_free_ad_spins');

      if (freeSpins <= 0 && adSpins <= 0) {
        return res.status(402).json({ error: 'NO_SPINS_AVAILABLE' });
      }

      const spinType = freeSpins > 0 ? 'default_free_spins' : 'default_free_ad_spins';
      await UserModel.removeBalance(user.deviceId, spinType, 1);
      ...
      if (selectedReward) {
        if (selectedReward.Type === 'Currency') {
          await UserModel.addBalance(user.deviceId, selectedReward.CurrencyType, selectedReward.Amount);
  ...
  static async giveCurrency(req, res) {
    try {
      ...
      const validCurrencies = ['gems', 'Gold', 'dust', 'default_free_spins', 'default_free_ad_spins'];
...
class TournamentXController {
  static tournaments = [
    {
      ...
      nameKey: "Legendary BlockDash Only Punch",
      descriptionKey: "Stumble into greatness in this intense 1v1 Legendary BlockDash battle! We’ve cranked up the stakes with exclusive rewards that only the true champions can earn. If you’ve got the skills to navigate the lasers and the guts to pay the fee, the glory is yours for the taking. Do you have what it takes to be the last one standing?",
      ...
      prizeBannerColour: "#9e4400",
      headerColour: "#a13300",
      mapListGradientColourTop: "#d35f00",
      mapListGradientColourBottom: "#c32000",
      listPriority: 1,
      minPlayers: 2,
      maxPlayers: 2,
      maxRounds: 1,
      ...
      entryCurrencyCost: 25,
      areEmotesRestricted: false,
      prohibitedEmotes: [8, 13, 55, 122, 123, 124],
      detailsPanelBorderColourTop: "#803100",
      detailsPanelBorderColourBottom: "#441a00",
      colourData: {
        detailsPanelMainColour: "#a03b00",
        detailsPanelBorderColour: "#803a00",
        headerGradientRight: "#663100",
        headerGradientLeft: "#994200",
        infoWidgetsGradientRight: "#663000",
        infoWidgetsGradientLeft: "#441f00",
        infoWidgetsBorderColour: "#803500"
      },
      rounds: [
        {
          roundOrder: 1,
      ...
      awards: [
            { placementRangeLowest: 1, placementRangeHighest: 1, awardId: 1, type: "gems", amount: 50}
          ],
      players: [],
      partys: []
    },
    {
      ...
      startTime: new Date(),
      endTime: new Date(Date.now() + 62 * 24 * 60 * 60 * 1000),
      nameKey: "BlockDash Only Punch",
      descriptionKey: "Test your skills in a high-stakes 1v1 on the iconic BlockDash map. We’ve raised the buy-in and the rewards—now it’s up to you to prove you're worth the investment. Navigate the obstacles, outlast your opponent, and secure the win. The blocks are moving. Are you ready?",
      ...
      prizeBannerColour: "#00229e",
      headerColour: "#0800a1",
      mapListGradientColourTop: "#004dd3",
      mapListGradientColourBottom: "#0d00c3",
      listPriority: 1,
      minPlayers: 2,
      maxPlayers: 2,
      maxRounds: 1,
      ...
      areEmotesRestricted: false,
      prohibitedEmotes: [8, 13, 55, 122, 123, 124],
      detailsPanelBorderColourTop: "#003780",
      detailsPanelBorderColourBottom: "#000044",
      colourData: {
        detailsPanelMainColour: "#000da0",
        detailsPanelBorderColour: "#000280",
        headerGradientRight: "#003066",
        headerGradientLeft: "#001499",
        infoWidgetsGradientRight: "#002a66",
        infoWidgetsGradientLeft: "#000a44",
        infoWidgetsBorderColour: "#040080"
      },
      rounds: [
        {
          roundOrder: 1,
    ...
    },
    {
      ...
      startTime: new Date(),
      endTime: new Date(Date.now() + 62 * 24 * 60 * 60 * 1000),
      nameKey: "LaserTracer Only Banana",
      descriptionKey: "Get ready for a high-stakes, neon-drenched showdown in the LaserTracer Only Banana tournament! This is a pure test of agility and nerves, stripped down to the essentials for the ultimate competitive experience.",
      ...
      prizeBannerColour: "#0a7700",
      headerColour: "#0f9900",
      mapListGradientColourTop: "#33ff55ff",
      mapListGradientColourBottom: "#003303",
      listPriority: 0,
      minPlayers: 2,
      maxPlayers: 2,
      maxRounds: 1,
      ...
      areEmotesRestricted: false,
      prohibitedEmotes: [7],
      detailsPanelBorderColourTop: "#0b8000",
      detailsPanelBorderColourBottom: "#004401",
      colourData: {
        detailsPanelMainColour: "#006607",
        detailsPanelBorderColour: "rgb(54, 255, 51)",
        headerGradientRight: "#07cc00ff",
        headerGradientLeft: "rgb(51, 255, 65)",
  ...
  static async join(req, res) {
    try {
      ...
      if (!tournament.isEnabled) {
        return res.status(400).json({ message: "tournament disabled" });
      }

      const now = new Date();
      if (now < tournament.startTime || now > tournament.endTime) {
        return res.status(400).json({ message: "The tournament is not currently active." });
      }
      ...
      if (tournament.entryCurrencyCost > 0) {
        ...
        if (userBalance < tournament.entryCurrencyCost) {
          return res.status(400).json({
            message: `insufficient balance of ${tournament.entryCurrencyType}`
          });
        }
      ...
      console.log(`I put the ${user.username} at the match ${partyId} in the tournament ${tournament.nameKey} with id ${tournament.id} and token ${entryToken}`);
      res.status(200).json(response);
    } catch (err) {
      console.error("error entering the tournament:", err);
      res.status(500).json({ message: "internal server error" });
    }
  }
  ...
  static async leave(req, res) {
    try {
      ...
      res.status(200).json({ message: "it went out" });
    } catch (err) {
      console.error("tournamentx", "error when leaving the tournament:", err);
      res.status(500).json({ message: "internal server error" });
    }
  }
...
static async finish(req, res) {
  try {
    ...
    if (typeof Round === 'undefined') {
      return res.status(400).json({ mensagem: "need the round" });
    }

    if (!user || !user.stumbleId || !user.userProfile) {
      return res.status(400).json({ mensagem: "invalid user" });
    }

    const roundResult = parseInt(Round);
    if (isNaN(roundResult)) {
      return res.status(400).json({ mensagem: "invalid round" });
    }
    ...
    if (crownsChange !== 0) {
      await UserModel.addBalance(user.deviceId, "crowns", crownsChange);
    }

    await TournamentModel.addPoints(user.stumbleId, pointsChange);

    console.log("I finished the round of " + user.username);
    ...
    console.error("error when finishing tourx round:", err);
    ...
    static async joinTournament(req, res) {
        try {
            ...
            if (tournament.entryFee > 0) {
                ...
                await UserModel.removeBalance(user.deviceId, 'gems', tournament.entryFee);
            }
